...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, K: int, d: int, bn_decay: float, adj,device):
...
class SpatioTemporalAttention(nn.Module):
    ...
    def __init__(self, K: int, d: int, bn_decay: float, mask: bool,adj,device):
        super(SpatioTemporalAttention, self).__init__()
        self._spatial_attention = SpatialAttention(K, d, bn_decay,adj,device)
    ...
    def forward(self, X: torch.FloatTensor, STE: torch.FloatTensor) -> torch.FloatTensor:
        ...
        HT = self._temporal_attention(HS, STE)
...
class GMAN3(nn.Module):
    ...
    def __init__(self, L: int, K: int, d: int, num_his: int, bn_decay: float, steps_per_day: int, use_bias: bool, mask: bool, SE,adj,device):
        super(GMAN3, self).__init__()
        self.SE = SE
        self.device = device
        D = K * d
        ...
        self._st_att_block1 = nn.ModuleList(
            [SpatioTemporalAttention(K, d, bn_decay, mask,adj,device) for _ in range(L)])
