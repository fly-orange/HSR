...
class GraphConstructor(nn.Module):
    ...
    def __init__(self, nnodes: int, k: int, dim: int, alpha: float=0.5, xd: Optional[int]=None):
...
#        _,Wv[:,i:i+1,:] = self.Wv(x[:,:,i:i+1],hidden_init_v)
        Wk = Wk.transpose(1,2)
        score = torch.bmm(Wq, Wk)
        score = score/np.sqrt(K)
...
#         score = score.masked_fill(future_mask, float('-inf'))
        # score =  torch.mul(score, A)
        attention = F.softmax(score,dim=-1)  # (B, N, N)
        # attention = torch.mul(attention,A)
...
class GraphTrans(nn.Module):
    def __init__(self,
        ...
        self._fully_connected_2 = FullyConnected(input_dims=[D, D], units=[D, 1], activations=[F.relu, None],
                                                 bn_decay=bn_decay)
        self._st_embedding = SpatioTemporalEmbedding(D =D, bn_decay = bn_decay, steps_per_day = 288 ,device = device, use_bias= False)

        self.spatial_encoder = SpatialAttention(D,D,1,device)
        
        self.encoder = EncoderRNN(2*D,D,1)
        self.decoder = DecoderRNN(D,D,1) 
        
        self.temporal_decoder = TemporalAttention(K, (D//K), bn_decay, True, device)


    def forward(self, X: torch.Tensor, TE : torch.Tensor)->torch.Tensor:
        
        # A = self._graph_construct(torch.arange(self.nnodes).to(self.device))
        # A = 0
        if X.dim() == 3:
            X = torch.unsqueeze(X,-1)  # (B, T_his, N, 1)
        X = self._fully_connected_1(X)  # (B, T_his, N, D)
        N = X.shape[2]
        ...
        # y = self._fully_connected_2(future_decoding)

        return y


class GCGT(nn.Module):
    def __init__(self,
                 nnodes: int,
                 ne:int,
                 p_length: int,
        ...
        self._fully_connected_2 = FullyConnected(input_dims=[D, D], units=[D, 1], activations=[F.relu, None],
                                                 bn_decay=bn_decay)
        self._st_embedding = SpatioTemporalEmbedding(D =D, bn_decay = bn_decay, steps_per_day = 288 ,device = device, use_bias= False)
        self._graph_construct= GraphConstructor(nnodes,ne,dim=64)
    ...
    def forward(self, X: torch.Tensor, TE : torch.Tensor)->torch.Tensor:
        ...
        T_his = X.shape[1]
        T_pred = TE.shape[1] - X.shape[1]
        X_embedding = torch.cat([ self.spatial_encoder(X[:,i*self.p_length:i*self.p_length+self.p_length,:],A) for i in range(X.shape[1]//self.p_length)],dim = 1)  # (B, T_his, N, D)
...
class GraphTrans1(nn.Module):
    def __init__(self,
        ...
        ##block
        self._fully_connected_1 = FullyConnected(input_dims=[2, D], units=[D, D], activations=[F.relu, None],
                                                 bn_decay=bn_decay)
        self._fully_connected_2 = FullyConnected(input_dims=[D, D], units=[D, 1], activations=[F.relu, None],
                                                 bn_decay=bn_decay)
        self._st_embedding = SpatioTemporalEmbedding(D =D, bn_decay = bn_decay, steps_per_day = 288 ,device = device, use_bias= False)
...
class GraphTrans3(nn.Module):
    def __init__(self,
        ...
        self._fully_connected_2 = FullyConnected(input_dims=[D, D], units=[D, 1], activations=[F.relu, None],
                                                 bn_decay=bn_decay)
        self._fully_connected_3 = FullyConnected(input_dims=P, units=Q, activations= None,
                                                 bn_decay=bn_decay)                        
        self._st_embedding = SpatioTemporalEmbedding(D =D, bn_decay = bn_decay, steps_per_day = 288 ,device = device, use_bias= False)
    ...
    def forward(self, X: torch.Tensor, TE : torch.Tensor)->torch.Tensor:
        ...
        # X_his = torch.cat((X_embedding , TE_his),dim = -1)  #(B, T_his, N, 2*D)
        X_his = X_embedding # (B, T_his, N, 2*D)
       
        ##temporal encoder
        X_fut = self._fully_connected_3(X_his.permute(0,3,2,1)).permute(0,3,2,1) # (B, T_pred, N, D)
        # X_his = torch.squeeze(torch.cat(X_his.chunk(N,dim=2),dim=0)) #(B*N, T， D)
        ...
        ##temporal decoder
        embedding2 = self.temporal_decoder(X_fut) # ((B, T_his + T_pred, N, D))
        # embedding2 = embedding
        future_decoding = embedding2  # ((B, T_pred, N, D))
