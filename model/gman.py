...
class SpatioTemporalEmbedding(nn.Module):
    ...
    def __init__(self, D: int, bn_decay: float, steps_per_day: int, device, use_bias: bool=True):
        ...
        self._fully_connected_te = FullyConnected(
            input_dims=[84, D], units=[D, D], activations=[F.relu, None],
            bn_decay=bn_decay, use_bias=use_bias)
        self.device = device
    ...
    def forward(self, SE: torch.FloatTensor, TE: torch.FloatTensor, T: int) -> torch.FloatTensor:
        ...
        minofhour = torch.empty(TE.shape[0], TE.shape[1], 60)
        hourofday = torch.empty(TE.shape[0], TE.shape[1], 24)
        for i in range(TE.shape[0]):
            minofhour[i] = F.one_hot(TE[..., 0][i].to(torch.int64) % 60, 60)
        for j in range(TE.shape[0]):
            hourofday[j] = F.one_hot(TE[..., 1][j].to(torch.int64) % 24, 24)
        TE = torch.cat((minofhour, hourofday), dim=-1).to(self.device)
        TE = TE.unsqueeze(dim=2)
        TE = self._fully_connected_te(TE)
        del minofhour, hourofday
        return SE + TE
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor, STE: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # mask = self.adj.unsqueeze(dim=0).unsqueeze(dim=0)
...
class GMAN(nn.Module):
    ...
    def __init__(self, L: int, K: int, d: int, num_his: int, bn_decay: float, steps_per_day: int, use_bias: bool, mask: bool, SE,device):
